...
export const amlRoutes = ({ isAdmin, accessibleModules }: RouteContext) => {
  const routes: JSX.Element[] = [];

  if (isAdmin && accessibleModules?.includes("kyc_aml")) {
    routes.push(
      <Route key="risk" path="/aml/risk" element={layout(<RiskEngine />)} />,
      <Route
        key="tx"
        path="/aml/transactions"
        element={layout(<TransactionMonitoring />)}
      />,
      <Route key="sar" path="/aml/sar" element={layout(<STR />)} />,
      <Route
        key="watchlist"
        path="/aml/watchlist"
        element={layout(<WatchlistManagement />)}
      />,
      <Route
        key="compliance"
        path="/aml/compliance"
