...
export interface Invoice {
  ...
  // Set only on an instalment invoice generated by a payment plan.
  instalmentPlanId: string | null;
  instalmentIndex: number | null;
...
export const cancelInvoice = async (
  ...
  unwrap(await api.post(`/finance/invoices/${id}/cancel`, { reason }));
...
// ── Payment plans ─────────────────────────────────────────────
//
// Each instalment IS a real invoice now (see the backend redesign) —
// there's no separate "instalment status" to track here. `invoice`
// is the linked instalment invoice's live state (stage, payable,
// paidAmount), joined server-side, so the plan status is always
// exactly what the invoice list would also show — never a second
// copy that can drift out of sync.
export interface InstalmentInvoiceSummary {
  _id: string;
  ref: string;
  stage: InvoiceStage;
  payable: number;
...
export interface Instalment {
  ...
  amount: number;
  invoiceId: string;
  invoice: InstalmentInvoiceSummary | null;
}
...
// Cancels the original (bulk) invoice — reversing its GL posting if
// it had already been sent — and generates one Draft instalment
// invoice per instalment. Nothing is sent to the client until the
// plan is approved (see approvePaymentPlan) and each instalment's
// own due date arrives.
...
// One-time sign-off for the whole plan: pushes every Draft instalment
// invoice through submit → approve in one call. From then on, each
// instalment is sent automatically, on its own due date, by the daily
// cron — no more manual "send" per instalment.
export const approvePaymentPlan = async (
  planId: string,
): Promise<PaymentPlan> =>
  unwrap(await api.post(`/finance/payment-plans/${planId}/approve`));
