...
export function ClosureTab({ mandate }: { mandate: Mandate }) {
  ...
  const closureMut = useMutation({
    ...
    onError: (err: any) =>
      toast({
        title: "Couldn't update checklist item",
  ...
  return (
    <div className="space-y-3">
      ...
      {mandate.closureChecklist.map((c) => (
        ...
        >
          <Checkbox
            checked={c.done}
            onCheckedChange={(v) =>
              closureMut.mutate({ itemId: c._id, done: !!v })
            }
          />
          {c.label}
        </label>
      ...
      <Button
        disabled={
          !mandate.closureChecklist.every((c) => c.done) || closeMut.isPending
        }
