...
import { Search, Send, Eye, Edit2, MoreHorizontal } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export default function Clients() {
  ...
  const handleResendLink = (client: Client) => {
    toast({
      title: "Link Resent",
      description: `Onboarding link resent to ${client.email}`,
    });
  };
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem asChild>
                          <Link to={`/clients/${c.id}`}>
                            <Eye className="h-4 w-4 mr-2" /> View Client
                          </Link>
                        </DropdownMenuItem>
                        {c.status === "Invited" && (
                          <DropdownMenuItem onClick={() => handleResendLink(c)}>
                            <Send className="h-4 w-4 mr-2" /> Resend Onboarding
                            Link
                          </DropdownMenuItem>
                        )}
                        <DropdownMenuItem>
